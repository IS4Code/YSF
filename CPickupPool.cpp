	...
	if(m_bStreamingEnabled)
	{
		for(WORD playerid = 0; playerid != MAX_PLAYERS; playerid++)
		{
			...
			for(PickupMap::iterator p = pPlayerData[playerid]->ClientPlayerPickups.begin(); p != pPlayerData[playerid]->ClientPlayerPickups.end(); p++)
			{
				...
				// Stream in
				if(distance < 300.0f && pNetGame->pPlayerPool->dwVirtualWorld[playerid] == p->second->iWorld && !pPlayerData[playerid]->bClientPickupStreamedIn[p->first])
				{
				...
				// Stream out
				else if((distance > 300.0f || pNetGame->pPlayerPool->dwVirtualWorld[playerid] != p->second->iWorld) && pPlayerData[playerid]->bClientPickupStreamedIn[p->first])
				{
