...
{
	if (!CServer::Get()->IsInitialized()) return INT_MIN; // If unknown server version
	if (!CAddress::VAR_pRestartWaitTime) return 0;
...
{
	if (CScriptParams::Get()->Setup(3, std::string("CallFunctionInScript"), CScriptParams::MORE_PARAMETER_ALLOWED, amx, params)) return CScriptParams::Get()->HandleError();
...
AMX_DECLARE_NATIVE(Natives::YSF_DestroyPlayerObject)
{
	...
	std::map<WORD, std::map<BYTE, std::string> >::iterator it = pPlayerData[playerid]->m_PlayerObjectMaterialText.find(objectid);
...
AMX_DECLARE_NATIVE(Natives::YSF_DestroyVehicle)
{
	...
	if(pDestroyVehicle(amx, params))
	{
		...
		std::map<int, CVehicleSpawn>::iterator v = CServer::Get()->vehicleSpawnData.find(vehicleid);
...
{
	if (CScriptParams::Get()->Setup(3, std::string("TextDrawSetStringForPlayer"), static_cast<CScriptParams::Flags>(CScriptParams::LOADED | CScriptParams::MORE_PARAMETER_ALLOWED), amx, params)) return CScriptParams::Get()->HandleError();
...
AMX_DECLARE_NATIVE(Natives::YSF_AttachPlayerObjectToPlayer)
{
	...
	pAddon->creation_timepoint = GetTickCount();
